...
| State | In memory | **Postgres** (a Supabase instance today): OAuth tokens (encrypted), triggers, prompts, preferences, activity log |
...
| Data store | Postgres, hosted on Supabase today: `oauth_tokens` (ciphertext), `oauth_states`, `integrations`, `jira_triggers`, `jira_prompts`, `release_calendar`, `user_preferences`, `activity_log`, `audit_events`, `app_settings`. Reached over the Postgres protocol with a connection string (Sept 2026); the Supabase service-role API key was **revoked on 11 Sept 2026**, along with the legacy `anon` / `service_role` JWTs, so no credential can reach this data over HTTP any more. RLS enabled on all tables, no policies, which blocks Supabase's public anon keys. Moving to another Postgres is a connection-string change plus a dump/restore — spec §12.7 |
| Secrets (Render env) | Slack bot + app tokens, signing secret; Jira service-account email + API token; Atlassian OAuth client id + secret; database connection string; token-encryption key; Azure OpenAI key/endpoint; admin Slack ids; ops channel id |
...
| External data flows | Slack ⇄ bot; bot → Jira Cloud (as the user; service account for reads); bot → Azure OpenAI (prompts in spec §8); bot ⇄ Postgres (TLS) |
