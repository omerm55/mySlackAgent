...
| R2 | **OAuth tokens stored in plaintext** | `oauth_tokens` holds access *and refresh* tokens (`offline_access`) for every connected user, readable with the Supabase secret key, which the app holds and which bypasses RLS. A Supabase or Render env leak = long-lived Jira write access as every connected user. | ✅ **Done 2026-09-10** — AES-256-GCM at the application level (`TOKEN_ENCRYPTION_KEY`, held only in Render; Supabase sees ciphertext); legacy rows re-encrypted on first start; boot refuses to run with encrypted rows and no key; RLS enabled on all tables with no policies (`supabase/rls.sql`); **Disconnect** in App Home; rotation runbooks in spec §12.5. **Manual, pending: add the key in Render, run `rls.sql`, rotate the Supabase secret key.** |
| R3 | **OAuth `state` is the Slack user id** | Predictable, no nonce, no expiry. An attacker can complete the consent flow with *their* Atlassian account and `state=<victim>`, binding their Jira identity to the victim's Slack account: the victim's subsequent clicks are executed and audited as the attacker (audit poisoning), or vice-versa. | ✅ **Done 2026-09-10** — 256-bit random `state` in `oauth_states`, single-use (atomic conditional update), 24 h expiry; a reused/stale/forged link gets a 400 page and nothing is stored. Migration `supabase/oauth_states.sql`. |
...
| Data store | Supabase Postgres: `oauth_tokens`, `oauth_states`, `integrations`, `jira_triggers`, `jira_prompts`, `release_calendar`, `user_preferences`, `activity_log`; accessed with the secret (service-role) key; RLS enabled, no policies (`rls.sql`); OAuth tokens stored as AES-256-GCM ciphertext |
| Secrets (Render env) | Slack bot + app tokens, signing secret; Jira service-account email + API token; Atlassian OAuth client id + secret; Supabase URL + secret key; **token encryption key** (+ previous during rotation); Azure OpenAI key/endpoint; admin Slack ids; ops channel id |
